...
var _Game_Enemy_prototype_selectAllActions = Game_Enemy.prototype.selectAllActions;
    Game_Enemy.prototype.selectAllActions = function (actionList) {
        ...
        for (var i = 0; i < this.numActions(); i++) {
            var test = this.selectAction(actionListbk, ratingZero);
            this.action(i).setEnemyAction(test);
                actionListbk2.some(function(item,index){
                    if(item.skillId === test.skillId && !$dataSkills[test.skillId].note.match(/<CEPatternList>/)){
